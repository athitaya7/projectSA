...
# เชื่อมฐานข้อมูล MySQL โดยตรง
db = mysql.connector.connect(
    host="127.0.0.1",
    user="devuser",
    password="DevPass123!",              # ใส่รหัสผ่านของคุณ
    database="hrms"  # เช่น 'projectdb'
)
# ---------- ดึงข้อมูลหลัก ----------
df_emp = pd.read_sql("SELECT * FROM employees", db)
df_pay = pd.read_sql("SELECT * FROM payroll", db)
df_perf = pd.read_sql("SELECT * FROM performance_reviews", db)
db.close()
